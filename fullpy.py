...
RESULT = {
    leg:
    {topic:
    {mod: KAPPAS[f"{leg}_{topic}_{mod}"]
        for mod in RESULT[leg][topic]}
    for topic in RESULT[leg]}
for leg in RESULT
            }
...
import numpy as np
Average = np.array([i for i in RESDF.values.reshape(-1) if not np.isnan(i) ]).mean().item()

sns.heatmap(RESDF, annot=True, cmap="coolwarm")
import matplotlib.pyplot as plt
plt.title(f"Average = {Average:.4f}")
